...
void CRenderWidget::mouseMoveEvent(QMouseEvent* pEvent)
{
    if( m_pSampleMgr && m_pSampleMgr->GetCurrentSample() )
    {
        g_Event.Position = XSE::CPoint( (i32)pEvent->pos().x(), (i32)pEvent->pos().y() );
        g_Event.Delta = g_lastMousePos.Sub( g_Event.Position );
...
{
    g_lastMousePos = XSE::CPoint( (i32)pEvent->pos().x(), (i32)pEvent->pos().y() );
