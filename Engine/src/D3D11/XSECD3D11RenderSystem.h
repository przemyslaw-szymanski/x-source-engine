		...
		class CRenderSystem : public XSE::IRenderSystem
		{
			...
			public:
				...
				virtual					~CRenderSystem();
						...
						RSHandleRef		CreateTexture(const STextureDesc& Desc) xst_implement;
						...
						IRenderSystem::UPDATE_OPTIONS_RESULT	UpdateOptions(const SRenderSystemSettings& Options);
			...
			protected:
				...
				xst_i	void		_SetRendererResourceHandleRefCount(RSHandlePtr pOut, u16 uCount);
				xst_i	u16			_GetRendererResourceHandleId(const RSHandleRef Handle);
				xst_i	u16			_GetRendererResourceHandleRefCount(const RSHandleRef Handle);
